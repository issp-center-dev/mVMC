...
static int MergeBFHopLists_real(const int *left, const int *leftCount,
  ...
  for (qpidx = 0; qpidx < NQPFull; qpidx++) {
    ...
    for (i = 0; i < leftCount[qpidx]; i++) {
      if (AppendBFHopIndex_real(dst, &mergedCount[qpidx], Nsize, left[qpidx * Nsite + i]) != 0) return 1;
    }
    for (i = 0; i < rightCount[qpidx]; i++) {
      if (AppendBFHopIndex_real(dst, &mergedCount[qpidx], Nsize, right[qpidx * Nsite + i]) != 0) return 1;
    }
  }
  return 0;
}
