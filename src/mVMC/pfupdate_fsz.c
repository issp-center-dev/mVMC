...
static int BF_FSZ_InvUpdateWorkSizeForK(const int k, size_t *complexCount,
  ...
  small *= 4;
  n2 = 2*kk;
  /* vec + w + matUV + correctionTmp + small matrix + inverse + GETRI work */
  if(kn > SIZE_MAX/6 || small > SIZE_MAX/2
      || 6*kn > SIZE_MAX-2*small
      || 6*kn+2*small > SIZE_MAX-n2) return 1;
  total = 6*kn+2*small+n2;
  *complexCount = total;
  *intCount = n2;
  return 0;
}
...
static BF_FSZ_InvUpdateResult PrepareInvMBF_fsz_rows_child(
    const int nAffected, const int *affected,
    const double complex *oldInvM, const size_t oldInvMQpStride,
    double complex *invMNew, const size_t newInvMQpStride,
    const int *eleIdx, const int *eleSpn, const int qpStart,
    const int qpidx, const double complex *candidateSlater,
    double complex *vec, double complex *w, double complex *smallMatrix,
  ...
  for(k=0;k<nAffected;k++) {
    ...
    const int rsi = eleIdx[row]+eleSpn[row]*Nsite;
    const double complex *sltE_i = sltE+(size_t)rsi*(size_t)Nsite2;
    for(j=0;j<nsize;j++) {
      double complex value = 0.0+0.0*I;
      for(i=0;i<nsize;i++) {
        const int rsk = eleIdx[i]+eleSpn[i]*Nsite;
        value += sltE_i[rsk]*newInv[(size_t)i*(size_t)nsize+j];
      }
      value -= (row == j) ? 1.0 : 0.0;
      {
...
BF_FSZ_InvUpdateResult PrepareInvMBF_fsz_rows_workspace(
  ...
  if(affected == NULL || oldInvM == NULL || invMNew == NULL
      ...
      || (qpNum > 1 && (oldInvMQpStride > SIZE_MAX/(qpNum-1)
          || newInvMQpStride > SIZE_MAX/(qpNum-1)))) {
    return BF_FSZ_InvResultValue(BF_FSZ_INV_UPDATE_INVALID_ARGUMENT,
        BF_FSZ_INV_STAGE_NONE,qpStart,0,0.0,0.0,0.0);
  }
  ...
  for(qpidx=0;qpidx<qpEnd-qpStart;qpidx++) {
    BF_FSZ_InvUpdateResult result = PrepareInvMBF_fsz_rows_child(
        nAffected,affected,oldInvM,oldInvMQpStride,invMNew,newInvMQpStride,
        eleIdx,eleSpn,qpStart,qpidx,candidateSlater,vec,w,smallMatrix,
