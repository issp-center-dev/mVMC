void makeham(struct BindStruct *X){
    ...
    for(int_i =0; int_i < X->Def.NInterAll; int_i++){
      ...
      /*Diagonal Fock term*/
#if Fock==1
      X->Large.Ham[sigma][site_1][site_2] += tmp*X->Large.G[tau][site_3][site_4];
      X->Large.Ham[tau][site_3][site_4]   += tmp*X->Large.G[sigma][site_1][site_2];
      if(sigma==tau){
        X->Large.Ham[sigma][site_1][site_4] += -1.0*tmp*X->Large.G[sigma][site_3][site_2];
        X->Large.Ham[sigma][site_3][site_2] += -1.0*tmp*X->Large.G[sigma][site_1][site_4];
      }
#endif
    }
