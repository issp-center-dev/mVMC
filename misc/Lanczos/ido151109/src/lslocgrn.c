...
void LSLocalQ(const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, int *eleProjCnt
    ,const int transStart, const int transEnd, MPI_Comm comm);
//void LSLocalQ_Store(const double sqrtw, const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, 
//    int *eleProjCnt ,const int transStart, const int transEnd, 
//    double *lsW, double *lsH, double *lsHcH, double *lsHkH);
void LSLocalQ_Store(const double sqrtw, const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, 
    int *eleProjCnt ,const int transStart, const int transEnd, double *lsHH, const int rank);
...
double calculateHK(const double h1, const double ip, int *eleIdx, int *eleCfg,
                   int *eleNum, int *eleProjCnt,const int transStart, const int transEnd);
...
void LSLocalQ(const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, int *eleProjCnt
  ...
  invSize     = 1.0/((double)size);
  //const double invSizeSqrt = 1.0/sqrt((double)size);

  StartTimer(80);
  e0 = CalculateHamiltonian0(eleNum); /* V */
  StopTimer(80);

  //h2 = h1*e0*invSize; /* HV = (V+K+W)V */
  h2 = h1*e0; /* HV = (V+K+W)V */
  h2 += calculateHW(h1,ip,eleIdx,eleCfg,eleNum,eleProjCnt);
  StartTimer(81);
  hk = calculateHK(h1,ip,eleIdx,eleCfg,eleNum,eleProjCnt,transStart, transEnd);
  StopTimer(81);
  StartTimer(82);
  if(size>1) {
    MPI_Allreduce(&hk, &hk2, 1, MPI_DOUBLE, MPI_SUM, comm);
    h2 += hk2;
  }else{
    h2 += hk;
  }
...
void LSLocalQ_Store(const double sqrtw, const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, 
  ...
  double e0,h2,hk,hk2;
  double hkh,hch;
  const double invSize = 1.0/(double)NSplitSize;
  ...
  //h2 = h1*e0*invSize; /* HV = (V+K+W)V */
  hch = h1*e0; /* HV = (V+K+W)V */
  //h2 += calculateHW(h1,ip,eleIdx,eleCfg,eleNum,eleProjCnt);
  StartTimer(81);
  hkh = calculateHK(h1,ip,eleIdx,eleCfg,eleNum,eleProjCnt,transStart, transEnd);
  StopTimer(81);

  /* calculate local Q (IQ) */
  lsHH[0] = sqrtw*invSize; /* I */
  lsHH[1] = sqrtw*h1*invSize;  /* H = V+K+W */
  //lsW[0] = sqrtw*invSize; /* I */
  //lsH[0] = sqrtw*h1*invSize;  /* H = V+K+W */

  /* calculate local Q (KQ) */
  lsHH[2] = sqrtw*hch*invSize;  /* Hc*H */
  lsHH[3+rank] = sqrtw*hkh;  /* Hk*H */
  //lsHcH[0] = sqrtw*hch*invSize;  /* Hc*H */
  //lsHkH[0] = sqrtw*hkh;  /* Hk*H */
...
void LSLocalCisAjs(const double h1, const double ip, int *eleIdx, int *eleCfg, int *eleNum, int *eleProjCnt) {
  ...
  double *lsLCisAjs = LSLCisAjs;
  double *localCisAjs = LocalCisAjs;
  int ri,rj,s;
  int idx;
...
double calculateHK(const double h1, const double ip, int *eleIdx, int *eleCfg,
  ...
  for(idx=transStart;idx<transEnd;idx++) {
    ri = Transfer[idx][0];
    rj = Transfer[idx][1];
    s  = Transfer[idx][2];
