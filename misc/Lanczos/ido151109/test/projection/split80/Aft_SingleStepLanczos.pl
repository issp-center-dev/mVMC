...
#===============Read input file====================
  for($i=0;$i<$Sample;$i++){
    #$cnt=0;
    $file=sprintf("zvo_ls_00%d.dat",$i);
    #print "$file \n";
    open(INPUTFILE,$file);
...
#=== calculating energy and variance ========================
  ...
  for($i=0;$i<$Sample;$i++){
    $Ave_H       += $r_H[$i];
    $Err_H       += $r_H[$i]**2;
    $Ave_V       += ($r_H2_1[$i]-$r_H[$i]**2)/$r_H[$i]**2;
    $Err_V       += ($r_H2_1[$i]-$r_H[$i]**2)**2/$r_H[$i]**4;
  ...
  $Have    = $Ave_H/(1.0*$Sample);
  $Have_err =  sqrt($Err_H/(1.0*($Sample-1.0))-$Sample/($Sample-1.0)*$Have**2);
  $Vave    = $Ave_V/(1.0*$Sample);
  $Vave_err =  sqrt($Err_V/(1.0*($Sample-1.0))-$Sample/($Sample-1.0)*$Vave**2);
