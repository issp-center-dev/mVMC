...
+------------------------------------------+--------------------------------------------------------------------+
| \*\*\*\_spinjastrow\_opt.dat             | 最適化されたスピンJastrow因子.                                     |
+------------------------------------------+--------------------------------------------------------------------+
...
+------------------------------------------+--------------------------------------------------------------------+
| zvo\_gc.dat                              | grand-canonical modeの粒子数moment.                                |
+------------------------------------------+--------------------------------------------------------------------+
...
+------------------------------------------+--------------------------------------------------------------------+
| xxx\_anomalousg\_yyy.dat                 | grand-canonical pair生成・消滅期待値.                              |
+------------------------------------------+--------------------------------------------------------------------+
| xxx\_twist\_yyy.dat                      | Twist演算子の期待値.                                               |
+------------------------------------------+--------------------------------------------------------------------+

Grand-canonical粒子数出力 (zvo\_gc.dat)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``NGrandCanonical=1`` のとき、この固定名fileへ
:math:`\langle N\rangle`、:math:`\langle N^2\rangle`、
:math:`\langle N^2\rangle-\langle N\rangle^2` の3列を出力します。
変分最適化ではSR stepごとに1行、物理量計算ではbinごとに1行を書きます。
ファイル名には ``CDataFileHead`` と ``NDataIdxStart`` を適用しません。同じ
directoryで別runを開始すると、以前の ``zvo_gc.dat`` は置き換えられます。
...
.. math:: \langle H \rangle, \langle H^2 \rangle, g_i, v_{ij}, v^s_{ij}, \alpha_{2nt}^{d(h)}, \alpha_{4nt}^{d(h)}, a_{i\sigma}, b_{k}, W_{i\sigma k}, f_{ij} \nonumber
...
Spin Jastrow因子出力ファイル(\*\*\*\_spinjastrow\_opt.dat)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

SR法で最適化されたSpin Jastrow因子が出力されます。
出力形式は :ref:`InputParam` のInSpinJastrow指定ファイルと同じです。
...
各最適化ステップごとに、
変分パラメータ数 ``Npara`` 、S行列の次元数 ``Msize`` 、 ``Orbital`` などの入力ファイルで最適化しないと指定した実パラメータ数 ``optCut`` 、 ``ModPara`` ファイルの ``DSROptRedCut`` の値により削減された実パラメータ数 ``diagCut`` 、S行列の対角成分の最大値 ``sDiagMax`` と最小値 ``sDiagMin`` 、変分パラメータの変化量の最大値 ``absRmax`` とそのパラメータインデックス ``imax`` が順に出力されます。
...
SR-CG (``NSRCG!=0``) の場合、ソルバ状態 ``info`` が
``imax`` の次の列として追加出力されます。
SR-CGの通常収束時、 ``info`` は反復回数です。
SR-CGが収束しない、または数値不安定になった場合、フォールバックの
有無に関わらず ``info = -(iter + 1)`` の負値が記録されます
(``iter`` は失敗を検出した反復回数)。
ただし、デフォルト (``NSRCGFallback=0``, ``NSRCGAbortOnFail=1``) では
失敗時に計算が終了するため、この行は出力されません。
``info`` が負値になるのはフォールバックや警告継続を有効にした場合のみです。
...
``NGrandCanonical=1`` かつ ``NVMCCalMode=1`` のとき、``AnomalousG`` で
指定したpair期待値を出力します。実際のfilenameは
``<CDataFileHead>_anomalousg_<index:03d>.dat`` です。``index`` は
``NDataIdxStart`` から始まり、物理量計算のbinごとに独立したfileを作成します。
出力例を次に示します。
...
各行は ``type s1 spin1 s2 spin2 Re Im`` の7列です。先頭5列は
``AnomalousG`` の入力順と演算子規約を保持し、末尾2列はsample weight平均と
MPI reduction後の実部・虚部です。``type=1`` はpair生成、``type=0`` はpair消滅を
表し、いずれも第2演算子を先に作用させます。

``NAnomalousG=0`` または最適化modeではfileを作成しません。
``NVMCCalMode=0`` で ``AnomalousG`` keywordを指定すると、count 0の場合を含めて
実行前に入力errorとなります。mVMCとHPhiではfilenameが異なりますが、入力演算子規約と
出力7列は直接cross-checkできます。
