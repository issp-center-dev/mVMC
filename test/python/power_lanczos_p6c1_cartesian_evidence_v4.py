#!/usr/bin/env python3
"""Recover and validate the P6-C1 v4 run after the v3 checker failure."""
...
DESIGN = PROJECT / "docs" / "2026-08-21-power-lanczos-p6c1-v4-recovery-design.md"
DEFAULT_INPUT = POLICIES / "power-lanczos-p6c1-cartesian-input-v4.txt.gz"
DEFAULT_SERIAL = POLICIES / "power-lanczos-p6c1-cartesian-serial-v4.txt.gz"
DEFAULT_MPI2 = POLICIES / "power-lanczos-p6c1-cartesian-mpi2-v4.txt.gz"
DEFAULT_MPI4 = POLICIES / "power-lanczos-p6c1-cartesian-mpi4-v4.txt.gz"
DEFAULT_EVIDENCE = POLICIES / "power-lanczos-p6c1-cartesian-evidence-v4.json"
ATTEMPT_CLAIM = (
    POLICIES / "power-lanczos-p6c1-cartesian-attempt-v4.json")
PRE_EXECUTION_MANIFEST = (
    POLICIES / "power-lanczos-p6c1-cartesian-pre-execution-manifest-v4.json")
DEFAULT_MANIFEST = (
    POLICIES / "power-lanczos-p6c1-cartesian-post-result-manifest-v4.json")
FAILURE_STDOUT = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-stdout-v4.txt.gz")
FAILURE_STDERR = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-stderr-v4.txt.gz")
FAILURE_MANIFEST = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-manifest-v4.json")
FAILURE_CANDIDATE_EVIDENCE = (
    POLICIES /
    "power-lanczos-p6c1-cartesian-failure-candidate-evidence-v4.json.gz")
FAILURE_CANDIDATE_MANIFEST = (
    POLICIES /
    "power-lanczos-p6c1-cartesian-failure-candidate-manifest-v4.json.gz")
CONTROL_VALIDATOR = (
    POLICIES / "validate_power_lanczos_p6c1_control_v4.py")
PARENT_V3_PRE_EXECUTION_MANIFEST = (
    POLICIES / "power-lanczos-p6c1-cartesian-pre-execution-manifest-v3.json")
PARENT_V3_ATTEMPT_CLAIM = (
    POLICIES / "power-lanczos-p6c1-cartesian-attempt-v3.json")
PARENT_V3_INPUT = (
    POLICIES / "power-lanczos-p6c1-cartesian-input-v3.txt.gz")
PARENT_V3_FAILURE_STDOUT = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-stdout-v3.txt.gz")
PARENT_V3_FAILURE_STDERR = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-stderr-v3.txt.gz")
PARENT_V3_FAILURE_MANIFEST = (
    POLICIES / "power-lanczos-p6c1-cartesian-failure-manifest-v3.json")
PARENT_V3_SUCCESS_ARTIFACTS = tuple(
    POLICIES / name for name in (
        "power-lanczos-p6c1-cartesian-serial-v3.txt.gz",
        "power-lanczos-p6c1-cartesian-mpi2-v3.txt.gz",
        "power-lanczos-p6c1-cartesian-mpi4-v3.txt.gz",
        "power-lanczos-p6c1-cartesian-evidence-v3.json",
        "power-lanczos-p6c1-cartesian-post-result-manifest-v3.json",
    ))

PARENT_V3_PRE_EXECUTION_MANIFEST_SHA256 = (
    "794e48f1cf46caa495c2ed2d5614d7bd41b3891d855fa01d4a650c5e93966905")
PARENT_V3_ATTEMPT_CLAIM_SHA256 = (
    "a78ad26b9f48f491bd2978afe81a2962010e5812e5bf2cad3f3b766f4b73557f")
PARENT_V3_INPUT_SHA256 = (
    "d0b4ced380cbf536ea55dc692eae157682f690ab914708108c77a688d1c34643")
PARENT_V3_FAILURE_STDOUT_SHA256 = (
    "81a8c27523d1eac65c8d0cb4e93d08d8dd7f6a98493350786412354a15ae7e43")
PARENT_V3_FAILURE_STDERR_SHA256 = (
    "9ceffb7310338057cfe71a4ae1e2c98d2c485d81cdef906532a801f457a38d64")
PARENT_V3_FAILURE_MANIFEST_SHA256 = (
    "2232875a59068886123b2ed2b0d1a1301103cde036105bc4e96939634043f644")
PARENT_V3_SERIAL_RAW_SHA256 = (
    "d0f72e28de562128ee6f2b483b8dd2b41d75a9b5fd62ce12389e76d0723560e4")
...
def validate_case(case: dict[str, Any], logical: dict[str, Any],
    ...
    for state_index, row in enumerate(case["rows"]):
        ...
        if row["scaled_state"] == 2:
            ...
            amplitude_exact = exact_vectors[0][state_index]
            require(Decimal(amplitude_exact["re_mid"]) == 0 and
                    Decimal(amplitude_exact["im_mid"]) == 0 and
                    math.isfinite(row["amplitude_error_bound"]),
                    f"{fixture_id}: amplitude numeric-zero exact-oracle proof")
        ...
        for vector_index, actual in enumerate(row["vectors"]):
            ...
            if metadata["state"] == 2:
                numeric_zero_count += 1
                require(Decimal(exact_record["re_mid"]) == 0 and
                        Decimal(exact_record["im_mid"]) == 0 and
                        math.isfinite(metadata["error_bound"]),
                        f"{fixture_id}: vector numeric-zero exact-oracle proof")
            elif expected == 0.0:
...
def load_parent_v3_serial() -> bytes:
    expected_files = {
        PARENT_V3_PRE_EXECUTION_MANIFEST:
            PARENT_V3_PRE_EXECUTION_MANIFEST_SHA256,
        PARENT_V3_ATTEMPT_CLAIM: PARENT_V3_ATTEMPT_CLAIM_SHA256,
        PARENT_V3_INPUT: PARENT_V3_INPUT_SHA256,
        PARENT_V3_FAILURE_STDOUT: PARENT_V3_FAILURE_STDOUT_SHA256,
        PARENT_V3_FAILURE_STDERR: PARENT_V3_FAILURE_STDERR_SHA256,
        PARENT_V3_FAILURE_MANIFEST: PARENT_V3_FAILURE_MANIFEST_SHA256,
    }
    require(all(path.is_file() and sha256_file(path) == expected_sha
                for path, expected_sha in expected_files.items()),
            "v3 recovery parent file identities")
    require(not any(path.exists() for path in PARENT_V3_SUCCESS_ARTIFACTS),
            "v3 success artifacts remain absent")
    parent_input = load_deterministic_gzip(PARENT_V3_INPUT, "v3 input")
    expected_input, _ = input_payload(preflight=False)
    require(parent_input == expected_input,
            "v3 recovery parent canonical input")
    raw = load_deterministic_gzip(
        PARENT_V3_FAILURE_STDOUT, "v3 failure serial stdout")
    require(sha256_bytes(raw) == PARENT_V3_SERIAL_RAW_SHA256 and
            load_deterministic_gzip(
                PARENT_V3_FAILURE_STDERR, "v3 failure stderr") == b"",
            "v3 recovery parent streams")
    failure_payload = PARENT_V3_FAILURE_MANIFEST.read_bytes()
    failure = json.loads(failure_payload)
    require(canonical_json(failure) == failure_payload and
            failure.get("manifest_id") ==
                "power_lanczos_p6c1_cartesian_failure_v3" and
            failure.get("status") == "observed_failed_attempt" and
            failure.get("failed_stage") == "serial" and
            failure.get("failed_world_size") == 1 and
            failure.get("process_started") is True and
            failure.get("return_code") == 0 and
            failure.get("completed_runs") == [] and
            failure.get("stored_failure_stdout_sha256") ==
                PARENT_V3_SERIAL_RAW_SHA256 and
            failure.get("stored_failure_stderr_sha256") ==
                sha256_bytes(b"") and
            failure.get("rerun_authorized") is False,
            "v3 recovery parent failure semantics")
    return raw
...
def build_attempt_claim(executable_sha256: str,
    ...
    return {
        "schema_version": 4,
        "attempt_id": "power_lanczos_p6c1_cartesian_attempt_v4",
        "status": "v3_serial_recovery_claimed_mpi_observation_pending",
        "production_authorized": False,
        "calibration_or_confirmation_result": False,
        ...
        "planned_run_order": ["recovered_v3_serial", "mpi2", "mpi4"],
        "planned_world_sizes": [1, 2, 4],
        "new_process_run_order": ["mpi2", "mpi4"],
        "serial_process_rerun": False,
        "serial_recovery_parent": {
            "failure_manifest_sha256":
                PARENT_V3_FAILURE_MANIFEST_SHA256,
            "failure_stdout_gzip_sha256":
                PARENT_V3_FAILURE_STDOUT_SHA256,
            "serial_raw_sha256": PARENT_V3_SERIAL_RAW_SHA256,
        },
        ...
        "success_next_phase": "p6_c1_v4_independent_recovery_review",
        "failure_next_phase": "p6_c1_v4_independent_failure_review",
    }
...
def build_cartesian_evidence(
    ...
    result = {
        "schema_version": 4,
        "evidence_id": "power_lanczos_p6c1_cartesian_evidence_v4",
        "status": "observed_v3_serial_recovery_method_validation_result",
        "calibration_or_confirmation_result": False,
        "production_authorized": False,
    ...
    result["runs"][0]["command_shape"] = [
        "<recovered-v3-failure-stdout>"]
    result["runs"][0]["process_rerun"] = False
    result["recovery_provenance"] = {
        "parent_pre_execution_manifest_sha256":
            PARENT_V3_PRE_EXECUTION_MANIFEST_SHA256,
        "parent_attempt_claim_sha256":
            PARENT_V3_ATTEMPT_CLAIM_SHA256,
        "parent_failure_manifest_sha256":
            PARENT_V3_FAILURE_MANIFEST_SHA256,
        "parent_failure_stdout_gzip_sha256":
            PARENT_V3_FAILURE_STDOUT_SHA256,
        "parent_serial_raw_sha256": PARENT_V3_SERIAL_RAW_SHA256,
        "serial_process_rerun": False,
...
def post_result_manifest(evidence: dict[str, Any],
    ...
    return {
        "schema_version": 4,
        "manifest_id": "power_lanczos_p6c1_cartesian_post_result_v4",
        ...
        "serial_process_rerun": False,
        "new_process_world_sizes": [2, 4],
        "next_phase": "p6_c1_v4_independent_recovery_review",
    }
...
def build_failure_manifest(
    ...
    return {
        "schema_version": 4,
        "manifest_id": "power_lanczos_p6c1_cartesian_failure_v4",
        ...
        "holdout_observation_state":
            "v3_serial_observed_recovery_incomplete",
...
def execute_cartesian(
    ...
    try:
        ...
        current_stage = "serial_recovery_validation"
        current_world_size = 1
        recovered_serial = load_parent_v3_serial()
        validate_full_payload(recovered_serial, 1)
        exclusive_write(DEFAULT_SERIAL, deterministic_gzip(recovered_serial))
        completed_runs.append("serial")
