...
import run_power_lanczos_p4f_v4_secondary_closure as closure
import runtest_power_lanczos_p4f_v3_result_validator as archive_fixture
import runtest_power_lanczos_p4f_v4_result_validator as fixture
...
def expect_failure(function, label):
    try:
        ...
        return
    raise AssertionError(f"negative v4 secondary closure accepted: {label}")
...
def write_fake_generator(path, mismatch=False):
    path.write_text(textwrap.dedent(f"""\
        ...
        evidence = json.loads((prepared /
            "primary/p4f_v4_confirmatory_evidence.json").read_text())
        decision = json.loads((prepared /
            "primary/p4f_v4_confirmatory_decision.json").read_text())
        metadata = dict(evidence["metadata"])
        metadata["generated_at"] = "2026-08-16 17:00 JST"
        ...
        for case in evidence["cases"]:
            ...
            value = ("v4 secondary " + case["trace"] + "\\n").encode()
            output.write_bytes(value)
            case["reparse_output_sha256"] = hashlib.sha256(value).hexdigest()
        if MISMATCH:
            evidence["cases"][0]["dimensions"][0]["prefixes"][0][
                "energy_se"] *= 2.0
        (args.output_dir / "p4f_v4_confirmatory_evidence.json").write_text(
            json.dumps(evidence, indent=2, allow_nan=False) + "\\n")
        (args.output_dir / "p4f_v4_confirmatory_decision.json").write_text(
    ...
    """), encoding="utf-8")
    return closure.common.sha256_path(path)
...
def patch_primary_provenance(result, expected, generator_path,
    ...
    evidence_path = confirmatory / "p4f_v4_confirmatory_evidence.json"
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update({
        "schema": 1,
        "generated_at": "2026-08-16 16:30 JST",
    ...
    evidence["metadata"] = metadata
    archive_fixture.write_json(metadata_path, metadata)
    archive_fixture.write_json(evidence_path, evidence)
    expected["generator_sha256"] = generator_sha
    fixture.refresh_checksums(result)


def make_prepared(root, stop, generator_path, generator_sha,
                  driver, driver_sha):
    result, expected = fixture.build_fixture(
        root / "fixture", hard_stop=stop)
    patch_primary_provenance(
        result, expected, generator_path, generator_sha, driver, driver_sha)
    archive = root / "result.tar.gz"
    checksum = archive_fixture.pack(result, archive)
    ...
    report = closure.preparer.prepare_secondary_inputs(
        archive, checksum, expected_path, prepared)
...
def run_case(root, stop, mismatch=False):
    ...
    closure.generator.__file__ = str(generator_path.resolve())
    driver = root / "p4f_matrix_reparse_driver"
    driver.write_text("#!/bin/sh\nexit 0\n", encoding="ascii")
    driver.chmod(0o755)
    driver_sha = closure.common.sha256_path(driver)
    ...
    if report["outcome"] != expected or report["p5_authorized"]:
        raise AssertionError("v4 secondary closure outcome mismatch")
    if (report["p4f_candidate_for_independent_review"] is not
            (expected == "GO")):
        raise AssertionError("v4 secondary review candidate mismatch")
    stored = json.loads(
        (output / "p4f_v4_secondary_closure.json").read_text())
    if stored["outcome"] != expected or \
            stored["closure_manifest_entry_count"] != 34 or \
            stored["expected_provenance_sha256"] != \
            preparation["expected_provenance_sha256"]:
        raise AssertionError("stored v4 secondary closure mismatch")
    closure.verify_tree_manifest(output, "closure_manifest.sha256")
    expect_failure(
        lambda: closure.run_secondary_closure(
            prepared, driver, output, driver_sha),
        "existing v4 closure output")
    expect_failure(
        lambda: closure.run_secondary_closure(
            prepared, driver, root / "bad-driver", "0" * 64),
        "v4 reparse driver SHA mismatch")
...
def main():
    ...
    saved = (
        ...
        closure.generator.__file__,
        closure.generator.SOLVER_POLICY_SHA256,
        closure.generator.CONFIRMATORY_POLICY_SHA256,
    )
    try:
        with tempfile.TemporaryDirectory(
                prefix="mvmc-p4f-v4-closure-go-", dir=scratch) as tmp:
            ...
            try:
                expect_failure(
                    lambda: closure.run_secondary_closure(
                        ...
                        driver_sha),
                    "v4 comparator SHA mismatch")
            finally:
                closure.COMPARATOR_SHA256 = original
        with tempfile.TemporaryDirectory(
                prefix="mvmc-p4f-v4-closure-stop-", dir=scratch) as tmp:
            run_case(pathlib.Path(tmp), stop=True)
        with tempfile.TemporaryDirectory(
                prefix="mvmc-p4f-v4-closure-mismatch-", dir=scratch) as tmp:
