      SUBROUTINE DSKTRD( UPLO, MODE, N, A, LDA, E, TAU, WORK, LWORK,
     $                   INFO )
*
...
*  =======
*
*  DSKTRD reduces a real skew-symmetric matrix A to skew-symmetric
...
*  ===============
*
*  The normal use for DSKTRD is to compute the tridiagonal form of
*  a skew-symmetric matrix under an orthogonal similarity transformation,
*  and chosen by setting MODE = 'N' ("normal" mode). The other
*  use of DSKTRD is the computation the Pfaffian of a skew-symmetric matrix,
...
*  or in the even columns (if UPLU = 'U') are properly computed by DSKTRD.
*
...
*  element not computed by DSKTRD.
*
*  =====================================================================
*
*     .. Parameters ..
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, DSKR2K, DSKTD2, DLASKTRD
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*        NOTE: I keep 'DSYTRD" here, as of course ILAENV has no information
*              about DSKTRD ...
...
*           the matrix
*
            CALL DLASKTRD( UPLO, MODE, I, NB, A, LDA, E, TAU, WORK,
     $                     LDWORK )
*
*           Update the unreduced submatrix A(1:i-npanel,1:i-npanel), using an
*           update of the form:  A := A + V*W^T - W*V^T
*
            CALL DSKR2K( UPLO, 'No transpose', I-NPANEL, NB, ONE,
     $                   A( 1, I-NPANEL+STEP ), LDA*STEP, WORK, LDWORK,
     $                   ONE, A, LDA )
*
...
*           the matrix
*
            CALL DLASKTRD( UPLO, MODE, N-I+1, NB, A( I, I ), LDA,
     $                     E( I ), TAU( I ), WORK, LDWORK )
*
*           Update the unreduced submatrix A(i+npanel:n,i+npanel:n), using
*           an update of the form:  A := A + V*W^T - W*V^T
*
            CALL DSKR2K( UPLO, 'No transpose', N-I-NPANEL+1, NB, ONE,
     $                   A( I+NPANEL, I ), LDA*STEP,
     $                   WORK( NPANEL+1 ), LDWORK, ONE,
...
*        Use unblocked code to reduce the last or only block
*
         CALL DSKTD2( UPLO, MODE, N-I+1, A( I, I ), LDA, E( I ),
     $                TAU( I ), IINFO )
      END IF
*
      WORK( 1 ) = LWKOPT
